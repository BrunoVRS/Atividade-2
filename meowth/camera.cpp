...
void Camera::dolly(float speed) {
  ...
  const glm::vec3 standard_translation{glm::vec3(0.0f, -angulo+0.5f, 0.0f)};
  const glm::vec3 forward{glm::normalize(m_at - m_eye)};

  if (angulo == 0.0f){
    const glm::vec3 forward{glm::normalize(m_at - m_eye)};
    const glm::vec3 cleaned_forward{glm::vec3(forward.x, 0.0f, forward.z)};   


    m_eye += forward * speed;
    m_at += cleaned_forward * speed;
    m_at.y = 0.5f;
  }
  else{
    const glm::vec3 forward{glm::normalize(m_at - m_eye)};
    ...
    //m_eye += forward * speed;
    m_at += forward * speed  + standard_translation/1000.0f ;
  }
  computeViewMatrix();
}
